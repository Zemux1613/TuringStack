...
public class Parser extends Phase {
    ...
    public void start() {
        // load tokens
        final List<Token> tokens = (List<Token>) AnalyseService.getStorage().get(Constants.EXTRACTED_TOKENS);
                ...
                if (!match) {
                    match = command
                            ...
                            .anyMatch(tokenCategories ->
                                    compareList(commandLine.tokens().stream().map(Token::category).toList(), tokenCategories));
                }
            }
            if (!match) {
                try {
     ...
     *
     * @param commandLine     Current command line which is checked by pattern maching with the command chain of the passed command.
     * @param command         Object of a command which is checked for equality and executed if it is equal.
